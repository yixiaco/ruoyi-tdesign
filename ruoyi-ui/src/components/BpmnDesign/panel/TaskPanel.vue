      ...
      <t-collapse v-model="currentCollapseItem" expand-icon-placement="right" borderless>
        ...
        <t-collapse-panel value="2">
          ...
          <div>
            ...
            <t-tabs placement="left" class="demo-tabs" default-value="1" @tab-click="taskTabClick">
              <t-tab-panel label="身份存储" value="1">
                <t-form-item label="分配人员">
                  <t-input v-model="assignee.userName" readonly @click="openSingleUserSelect">
                    <template #suffixIcon>
          ...
          <div>
            <t-form-item label="多实例类型">
              <t-select v-model="formData.multiInstanceType" @change="multiInstanceTypeChange">
                <t-option v-for="item in MultiInstanceType" :key="item.id" :value="item.value" :label="item.label">
                </t-option>
              </t-select>
            </t-form-item>
